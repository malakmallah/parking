...
// Pagination settings (applies only to list view)
$per_page = 10;
...
if ($action === 'list') {
    ...
    // Fetch current page
    // NOTE: $per_page and $offset are integers controlled by the server, safe to inject.
    $stmt = $pdo->prepare("
            ...
            <?php else: ?>
                ...
                <div class="card">
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge <?php echo $user['role'] === 'staff' ? 'bg-primary' : 'bg-success'; ?>">
                    ...
                    <div class="card-body pt-0">
                        <?php
                            // Helper to build pagination link preserving existing filters/search
                            function build_page_link($pageNum) {
                                $query = $_GET;
                                $query['page'] = $pageNum;
                                // Ensure action stays 'list'
                                $query['action'] = 'list';
                                return 'users.php?' . htmlspecialchars(http_build_query($query));
                            }

                            $maxButtons = 7; // max number of numeric buttons to show
                            $start = max(1, $page - 3);
                            $end = min($total_pages, $page + 3);
                            if (($end - $start + 1) < $maxButtons) {
                                $needed = $maxButtons - ($end - $start + 1);
                                $start = max(1, $start - $needed);
                                $end = min($total_pages, $end + $needed);
                            }
                        ?>
                        <nav aria-label="Users pagination">
                            <ul class="pagination justify-content-end mb-0">
                                <li class="page-item <?php echo $page <= 1 ? 'disabled' : ''; ?>">
                                    <a class="page-link" href="<?php echo $page > 1 ? build_page_link($page - 1) : '#'; ?>" tabindex="-1" aria-disabled="<?php echo $page <= 1 ? 'true' : 'false'; ?>">
                                        &laquo; Prev
                                    </a>
                                </li>

                                <?php if ($start > 1): ?>
                                    <li class="page-item"><a class="page-link" href="<?php echo build_page_link(1); ?>">1</a></li>
                                    <?php if ($start > 2): ?>
                                ...
                                <?php for ($i = $start; $i <= $end; $i++): ?>
                                    <li class="page-item <?php echo $i === $page ? 'active' : ''; ?>">
                                        <a class="page-link" href="<?php echo $i === $page ? '#' : build_page_link($i); ?>">
                                            <?php echo $i; ?>
                                        </a>
                                    </li>
                                <?php endfor; ?>
                                ...
                                <?php if ($end < $total_pages): ?>
                                    <?php if ($end < $total_pages - 1): ?>
                                        <li class="page-item disabled"><span class="page-link">...</span></li>
                                    <?php endif; ?>
                                    <li class="page-item"><a class="page-link" href="<?php echo build_page_link($total_pages); ?>"><?php echo $total_pages; ?></a></li>
                                <?php endif; ?>

                                <li class="page-item <?php echo $page >= $total_pages ? 'disabled' : ''; ?>">
                                    <a class="page-link" href="<?php echo $page < $total_pages ? build_page_link($page + 1) : '#'; ?>">
                                        Next &raquo;
                                    </a>
                                </li>
                            </ul>
